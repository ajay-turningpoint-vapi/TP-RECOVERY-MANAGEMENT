SELECT
    -- ref_code is the snapshot PRIMARY KEY. TRAN3 bill rows for opening
    -- balances (VchType = 1) can carry RefCode 0 / NULL, which would either
    -- fail the NOT NULL insert or collapse every such row onto one key via
    -- ON DUPLICATE KEY UPDATE. Fall back to a deterministic per-party key so
    -- each bill stays its own row.
    COALESCE(
        NULLIF(CONVERT(VARCHAR(64), B.RefCode), '0'),
        'OP-' + CONVERT(VARCHAR(20), B.MasterCode1) + '-' + LTRIM(RTRIM(ISNULL(B.No, '')))
    ) AS ref_code,
    B.MasterCode1 AS customer_id,
    M.Name AS customer_name,
    B.Date AS invoice_date,
    B.DueDate AS due_date,
    LTRIM(RTRIM(B.No)) AS invoice_no,
