...
class AppStore extends ChangeNotifier {
  ...
  List<AppTask> get myTasks => userRole == 'SALESPERSON' ? tasks.where((t) => t.ownerId == currentSalesmanId).toList() : tasks;
  // The real, server-tracked count (customers[].noAnswerAttempts) — this
  // used to be a local Map that was never actually written to anywhere,
  // so every customer always showed "Attempt 1" no matter how many real
  // No Answer outcomes had been recorded, and the form's "a Physical Visit
  // will be created automatically" warning could never fire on the call
  // that actually triggers it (server threshold is 2; the old dead counter
  // never moved past 1).
  ...
  // unanswered attempts before a Physical Visit is auto-recommended.
  static const int noAnswerThreshold = 2;
  ...
  List<Customer> get noFollowUpAccounts =>
  ...
  // Company-wide recovery target — 12% of total overdue exposure,
  ...
  double get teamTotalOutstanding => customers.fold(0.0, (sum, c) => sum + c.totalOutstanding);
  double get teamTotalDue => customers.fold(0.0, (sum, c) => sum + c.totalDue);
  ...
  double get totalOverdueAmount => (_dashboardReport['totalOverdueAmount'] as num?)?.toDouble() ?? 0.0;
  int get totalOverdueCustomerCount => (_dashboardReport['totalOverdueCustomerCount'] as int?) ?? 0;
  double get dueTodayPtpAmount => (_dashboardReport['dueTodayPtpAmount'] as num?)?.toDouble() ?? 0.0;
  int get dueTodayPtpCount => (_dashboardReport['dueTodayPtpCount'] as int?) ?? 0;
  int get overdueCustomersCount => (_dashboardReport['overdueCustomersCount'] as int?) ?? 0;
  double get overdueCustomersAmount => (_dashboardReport['overdueCustomersAmount'] as num?)?.toDouble() ?? 0.0;
  ...
  List<Customer> get topOverdueCustomers =>
      ((_dashboardReport['topOverdueCustomers'] as List?) ?? []).map((j) => Customer.fromJson(j as Map<String, dynamic>)).toList();
  ...
  double get totalReceivedAllTime => ptps.where((p) => p.status == PtpStatus.kept || p.status == PtpStatus.partiallyKept).fold(0.0, (s, p) => s + (p.amountReceived ?? 0));
  ...
  int get salesmenOverdueTargetsCount => salesmen.where((s) => (s['collectionAchievedPercent'] as int) < 60).length;
  List<AppTask> get physicalVisitsPendingReview =>
      tasks.where((t) => t.type == TaskType.physicalVisit && t.status == TaskStatus.completed && !t.reviewedByRE).toList();
  int get physicalVisitsPendingReviewCount => (_dashboardReport['physicalVisitsPendingReviewCount'] as int?) ?? 0;
  int get disputesAwaitingReviewCount => (_dashboardReport['disputesAwaitingReviewCount'] as int?) ?? 0;
  int get pendingTaskExtensionCount => (_dashboardReport['pendingTaskExtensionCount'] as int?) ?? 0;
  int get needsAttentionBadgeCount => (_dashboardReport['needsAttentionBadgeCount'] as int?) ?? 0;
  int get pendingOutcomeCorrectionCount => outcomeCorrectionRequests.where((r) => r.status == 'Pending').length;
  ...
  int get totalOpenTaskItemsCount {
    final plainTasks = tasks.where((t) {
      final isVisitPendingReview = t.type == TaskType.physicalVisit && t.status == TaskStatus.completed && !t.reviewedByRE;
      return t.approvalStatus != 'Pending' && !isVisitPendingReview && t.status != TaskStatus.completed;
    }).length;
  ...
  List<OutcomeCorrectionRequest> get pendingOutcomeCorrections => outcomeCorrectionRequests.where((r) => r.status == 'Pending').toList();
  ...
  List<OutcomeEditRequest> get pendingOutcomeEdits => outcomeEditRequests.where((r) => r.status == 'Pending').toList();
  ...
  /// requests, outcome edit requests, and outcome correction requests — see
  /// ApprovalsListScreen, which this count exists to match exactly.
  int get pendingApprovalsCount =>
      disputes.where((d) => d['status'] == 'Pending Approval').length +
      ptpCorrectionRequests.length +
      tasks.where((t) => t.approvalStatus == 'Pending').length +
  ...
  int get criticalApprovalsCount {
    ...
    return disputes.where((d) => d['status'] == 'Pending Approval' && d['priority'] == 'High').length +
        ptpCorrectionRequests.where((p) => escalated(p.customerId)).length +
        tasks.where((t) => t.approvalStatus == 'Pending' && (t.priority == 'Critical' || t.priority == 'High')).length +
  ...
  int get brokenPtpCount => customers.where((c) => c.reasonForAction.toLowerCase().contains('broken ptp')).length;
  int get mgmtInstructionCount => tasks.where((t) => t.type == TaskType.managementInstruction && t.status != TaskStatus.completed).length;
  int get physicalVisitDueCount => tasks.where((t) => t.type == TaskType.physicalVisit && t.status != TaskStatus.completed && t.deadline.isBefore(DateTime.now())).length;

  // RE control-layer aggregates
  double get ownerlessExposure => customers.where((c) => c.ownerMappingRequired).fold(0.0, (s, c) => s + c.totalDue);
  List<Customer> get ownerMappingRequiredCustomers => customers.where((c) => c.ownerMappingRequired).toList();
  List<Customer> get noValidNextActionCustomers => customers.where((c) => !c.hasValidNextAction && c.totalDue > 0).toList();
  List<EscalationCase> get openEscalationCases => escalationCases.where((e) => e.isOpen).toList();
  List<EscalationCase> get l4Cases => escalationCases.where((e) => e.isOpen && e.level == 'L4').toList();
  List<PromiseToPay> get brokenPtps => ptps.where((p) => p.status == PtpStatus.broken).toList();
  List<Customer> get highRiskAccounts => customers.where((c) => c.totalDue > 0 && c.creditHealthScore != null && c.creditHealthScore! < 70).toList();
  ...
  List<Customer> get atRiskAccounts {
    ...
    return customers.where((c) => c.totalDue > 0 && (c.escalationLevel != 'none' || brokenCustomerIds.contains(c.id) || c.oldestOverdueDays >= 60 || c.creditHealthBand == 'High' || c.creditHealthBand == 'Critical')).toList();
  }

  double get moneyAtRisk => atRiskAccounts.fold(0.0, (s, c) => s + c.totalDue);
  List<PromiseToPay> get syncPendingPtps => ptps.where((p) => p.status == PtpStatus.financialSyncPending).toList();
  List<PromiseToPay> get ptpCorrectionRequests => ptps.where((p) => p.correctionStatus == 'Pending').toList();
  int get unreadNotificationCount => notifications.where((n) => !n.read).length;
  int get overdueTaskCount => tasks.where((t) => t.isOverdue).length;
  ...
  void _handleSessionExpired() {
    ...
    _syncStatusPollTimer?.cancel();
    notifyListeners();
  }

  /// Polls whether the daily BUSY sync is currently running (see
  /// isSyncing's doc comment) — fast enough (10s) that the freeze overlay
  /// appears and disappears close to when the sync actually starts/ends,
  /// not e.g. 90s late like the notification poll can afford to be.
  void _startSyncStatusPolling() {
    _refreshSyncStatusFromApi();
    _syncStatusPollTimer?.cancel();
    _syncStatusPollTimer = Timer.periodic(const Duration(seconds: 10), (_) {
      _refreshSyncStatusFromApi();
    });
  }
  ...
  /// Requests the OS notification permission (Android 13+) once per
  /// session and starts foreground polling for new server-side
  /// notifications — there's no push (FCM) backend, so this poll is what
  /// actually surfaces a task/PTP/escalation alert as an OS notification
  /// rather than only the in-app bell badge. Deliberately not started
  /// until a real session exists — no point prompting for the permission
  ...
  void _startNotificationPolling() {
    ...
    _notificationPollTimer = Timer.periodic(const Duration(seconds: 90), (_) {
      _refreshNotificationsFromApi().then((_) => notifyListeners()).catchError((_) {
        // A poll failing (offline, momentary server hiccup) shouldn't
        // surface an error to the user — the next tick tries again, same
        // as any other silent background refresh.
      });
    });
  }
  ...
  Future<void> approveDispute(String id, String resolutionOwner, DateTime deadline, String description) async {
    final dispute = disputes.firstWhere((d) => d['id'] == id);
  ...
  Future<void> approveInternalAction(String taskId, {String? note}) async {
    final task = tasks.firstWhere((t) => t.id == taskId);
    await apiClient.approveInternalAction(taskId, {if (note != null && note.isNotEmpty) 'note': note});
    await _refreshTasksFromApi();
    await _refreshOneCustomerFromApi(task.customerId);
    notifyListeners();
  }

  Future<void> rejectInternalAction(String taskId, {String? reason}) async {
    final task = tasks.firstWhere((t) => t.id == taskId);
    await apiClient.rejectInternalAction(taskId, {if (reason != null && reason.isNotEmpty) 'reason': reason});
    await _refreshTasksFromApi();
    await _refreshOneCustomerFromApi(task.customerId);
    notifyListeners();
  }
  ...
  Future<void> assignManagementInstruction(String customerId, String salesmanId, String desc, DateTime deadline, {String priority = 'Critical', String taskType = 'managementInstruction'}) async {
  ...
  // ---------------------------------------------------------------------
  Future<void> requestPtpCorrection(String ptpId, double newAmount, DateTime newDate, String reason) async {
